...
const roles = ["Employee", "L&D Admin", "HR Manager", "System Admin"];

const permissions = [
  { module: "Course Management", access: [false, true, false, true] },
  { module: "Take Courses", access: [true, true, true, true] },
  { module: "View Global Reports", access: [false, true, true, true] },
  { module: "Manage Users", access: [false, false, true, true] },
  { module: "System Privileges", access: [false, false, false, true] },
  { module: "Edit Site Settings", access: [false, false, false, true] },
];

export default function LDPrivileges() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-3xl font-bold tracking-tight text-foreground">Role-Based Access Control</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Define access control lists (ACL) and feature flags system-wide.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 flex items-start gap-4">
        ...
        <div>
          <h3 className="text-sm font-semibold text-destructive">Administrative Warning</h3>
          <p className="text-xs text-muted-foreground mt-1">Changes made to these matrices propagate instantly across the entire tenant structure. Ensure you do not lock out System Administrators from managing active configurations.</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="bg-muted/30 border-b border-border">
            <tr>
              <th className="px-6 py-4 font-semibold text-foreground border-r border-border">Permission Module</th>
              {roles.map(role => (
